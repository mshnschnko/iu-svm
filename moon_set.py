...
from Kernel import GaussKernel, LinearKernel, PolyKernel, Kernel
...
def nonlinear_researh():
    ...
    kernels = [
        ...
        GaussKernel(sigma=4.0),
        GaussKernel(sigma=8.0),
        PolyKernel(degree=2, c=1.0),
        PolyKernel(degree=3, c=1.0),
        PolyKernel(degree=5, c=1.0),
    ]
    ...
    for kernel in tqdm(kernels):
        for C in C_list:
            ...
            if p.is_alive():
                print(f'TIMEOUT: {kernel} и C={C} выполняется слишком долго... завершение процесса...')
                p.terminate()
                p.join()
